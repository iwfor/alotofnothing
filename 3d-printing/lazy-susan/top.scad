...
ring(track_rad+track_width+track_wall_thickness*2+1, track_wall_thickness, track_wall_thickness+wheel_rad+6);

translate([0,0,track_wall_thickness+track_wall_height+5.25])
    rotate_extrude(convexity=10)
        translate([track_rad+track_width+track_wall_thickness*2+1,0,0])
            circle(r=0.75);
    ...
    for (i = [0:5]) {
        rotate([0,0,i*60]) translate([0,-7,0]) {
            difference() {
                ...
                for (a = [1/3,4/5]) {
                    translate([track_rad*a,7,-0.01]) cylinder(h=track_wall_thickness+1, r=4);
                }
            }
        }
    }
